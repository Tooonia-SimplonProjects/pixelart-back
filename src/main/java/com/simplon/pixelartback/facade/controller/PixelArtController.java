...
//  READ / GET one pixelArt by Id    GET /api/pixelart/{id_pixel_art}
    @GetMapping("/pixelart/{id_pixel_art}")
    public ResponseEntity<PixelArtDto> getPixelArtByUuid(@PathVariable("uuid") UUID uuid) throws Exception {
        return  ResponseEntity.ok(pixelArtService.getPixelArtByUuid(uuid));
    };
...
//    TODO: very likely there will be 2 @param here : userId and pixelArtDto !!! <= line 91 UsersApi.class!
    public ResponseEntity<PixelArtDto> createPixelArt(PixelArtDto pixelArtDto) throws Exception {
        ...
        val location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{uuid}")
                .buildAndExpand(createdPixelArtDto.getUuid()).toUri();
...
//    TODO: will there be 2 @param here : userId and pixelArtDto !!! <= line 99 UsersController or just one like in line 83 of ProjectController?
    public ResponseEntity<Void> deletePixelArt(UUID uuid) throws Exception {
        pixelArtService.deletePixelArt(uuid);
